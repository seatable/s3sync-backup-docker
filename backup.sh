...
SYNC_PARAMS="--config /root/.config/rclone/rclone.conf --stats 30m --stats-one-line --stats-log-level NOTICE --transfers=16 --checkers=16 --skip-links --s3-no-check-bucket --log-file="${lastLogfile}" --log-level=NOTICE --size-only"
